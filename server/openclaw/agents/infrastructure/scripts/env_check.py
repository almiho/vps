...
def check_gateway():
    ...
    if "rpc probe: ok" in out.lower():
        return True
    # Container mode: RPC probe fails because systemd unavailable.
    # If openclaw processes are running and web server is up, gateway is functional.
    proc = run("pgrep -f openclaw 2>/dev/null | wc -l").strip()
    proc_count = int(proc) if proc.isdigit() else 0
    return proc_count > 0 and check_web_server()
