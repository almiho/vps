...
def check_gateway():
    ...
    if proc_count > 0 and web_ok:
        # Processes running + web server up = gateway functional in container mode.
        # RPC probe fails because systemd is unavailable — not a real outage.
        return {"name": "gateway", "ok": True, "detail": f"Container mode: {proc_count} processes running, web server up. RPC probe N/A in this environment."}
    elif proc_count == 0:
        detail = (
...
def check_google_mcp():
    ...
    # Step 1: liveness probe — workspace-mcp exposes health at /
    try:
        with urllib.request.urlopen("http://127.0.0.1:8000/", timeout=5) as r:
            body_bytes = r.read()
            healthy = r.status == 200 and b"healthy" in body_bytes.lower()
    except Exception as e:
        return {"name": "google_mcp", "ok": False,
                "detail": (
                    f"Google MCP server unreachable on port 8000: {e}. "
                    "Gmail, Calendar, and Drive access are down. "
                    "Assessment: server process crashed or was never started. "
                    "→ Check workspace-mcp process and restart if needed."
                )}

    if not healthy:
        return {"name": "google_mcp", "ok": False,
                "detail": "Google MCP health endpoint returned non-healthy response. Server may be starting up or degraded."}

    # Step 2: MCP protocol check — initialize a session
    try:
        payload = _json.dumps({
            "jsonrpc": "2.0", "id": 1, "method": "initialize",
            "params": {"protocolVersion": "2024-11-05", "capabilities": {},
                       "clientInfo": {"name": "mony-healthcheck", "version": "1.0"}}
        }).encode()
        req = urllib.request.Request(
            "http://127.0.0.1:8000/mcp", data=payload,
        ...
        with urllib.request.urlopen(req, timeout=8) as r:
            body = r.read().decode()
        protocol_ok = '"protocolVersion"' in body and '"google_workspace"' in body
    except Exception as e:
        return {"name": "google_mcp", "ok": False,
                "detail": (
                    f"Google MCP liveness OK but MCP protocol handshake failed: {e}. "
                    "Server is up but not accepting sessions — may be misconfigured or overloaded."
                )}

    if not protocol_ok:
        return {"name": "google_mcp", "ok": False,
                "detail": "MCP initialize response missing expected fields — server may be returning errors."}

    return {"name": "google_mcp", "ok": True,
            "detail": "Google Workspace MCP healthy — Gmail, Calendar, Drive sessions available."}
...
def main():
    ...
    if n_fail > 0:
        ...
        health = "ok"
        summary = "All systems healthy. Gateway ✓ Dashboard ✓ Bus ✓ Disk ✓ Agents ✓ Scheduler ✓ Google MCP ✓"
